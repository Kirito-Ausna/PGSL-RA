...
def parse_align(align_sequence: str):
    sequences = []
    seq_list = []
    index = 0
    for line in align_sequence.splitlines():
        line = line.strip()
        if not line: # skip blank lines
            ...
            if line_content[0] == 'Q':
                offset = int(line_content[1]) - 1
            # logging.warning(f"the split line is {line_content}")
            sequences[index] += line_content[2]
            seq_list.append(list(sequences[index]))
            index = index + 1
    index_shift = 0
    for index, ch in enumerate(sequences[0]):
        if ch == '-':
            for i in range(2):
                seq_list[i].pop(index - index_shift)
            
            index_shift += 1

    sequences[0] = "".join(seq_list[0])
    sequences[1] = '-'*offset + "".join(seq_list[1])
