...
def _process_single_hit(
    ...
    try:
        ...
        #logging.warning(f"the file in {cif_path} doesn't exist")
...
class TemplateHitFeaturizer:
    ...
    def get_templates(
        ...
        # seq_len = len(template_features["template_sequence"][0])
        # logging.warning(f"The Length of Right Template is {seq_len}")
        # logging.warning(f"the alignment_dir is {alignment_dir}")
        # Include the template of our own
        # for f in os.listdir(alignment_dir):
        #     # logging.warning(f"the alignment file is {f}")
        #     path = os.path.join(alignment_dir, f)
        #     ext = os.path.splitext(f)[-1]
        #     if(ext == ".pdb"):
        #         # Create empty feature first
        #         # output_templates_sequence = []
        #         templates_all_atom_positions = []
        #         templates_all_atom_masks = []
        ...
        #             )
        #             # output_templates_sequence.append("-")
        #         # read the .align file to create mapping
        #         # from the original target sequence index to template sequence index
        #         f = os.path.splitext(f)[0] + ".ali"
        #         align_path = os.path.join(alignment_dir, f)
        #         with open(align_path, "r") as fp:
        #             sequences = parsers.parse_align(fp.read())
        #             temp_seqs = sequences[1]
        #             logging.warning(f"the length of template in {f} is {len(temp_seqs)}")
        #             logging.warning(f"the length of query in {f} is {len(sequences[0])}")
        #             logging.warning(f"the query is {sequences[0]}")
        #             logging.warning(f"the template is {temp_seqs}")
        #             if len(temp_seqs) != len(query_sequence):
        #                 continue

        #         mapping = {}
        #         index = 0
        #         for i, ch in enumerate(temp_seqs):
        #             if ch != '-':
        #                 mapping[i] = index
        #                 index = index + 1
        #         # chain_id = os.path.splitext(f)[0].split("_")[-1]
        #         chain_id = "A"
        #         with open(path) as file:
        #             pdb_str = file.read()
        #         protein_object = protein.from_pdb_string(pdb_str, chain_id)
        #         description = os.path.splitext(f)[0].lower()
        #         # read pdb extract raw feature
        #         all_atom_positions = protein_object.atom_positions.astype(np.float32)
        ...
        #         output_templates_sequence = temp_seqs
        #         templates_aatype = residue_constants.sequence_to_onehot(output_templates_sequence, residue_constants.HHBLITS_AA_TO_ID)
        #         result_feature = {}
        #         result_feature["template_all_atom_positions"] = np.array(templates_all_atom_positions)
        #         result_feature["template_all_atom_mask"] = np.array(templates_all_atom_masks)
        #         result_feature["template_sequence"] = output_templates_sequence.encode()
        #         result_feature["template_aatype"] = np.array(templates_aatype)
        #         result_feature["template_domain_names"] = description
        #         result_feature["template_sum_probs"] = template_features["template_sum_probs"][0]
        #         # append to the original template list
        #         num_hits += 1
        #         # check_list = ["template_domain_names","template_sequence","template_sum_probs"]
        #         # for k in check_list:
        #         #     logging.warning(f"the value of {k} is {template_features[k][0]}\n")
        #         #     logging.warning(f"while the value of my {k} is {result_feature[k]}\n")
        #         for k in template_features:
        #             template_features[k].append(result_feature[k])
                # seq_len = len(template_features["template_sequence"][0])
                # logging.warning(f"the length of template is {seq_len}")

        # template feature combine to form the num_template dimension
