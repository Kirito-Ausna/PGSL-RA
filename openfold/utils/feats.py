...
def pseudo_sidechain_beta_fn(aatype, all_atom_positions):
    aa_group = {1:['G'], 2:['A'], 3:['S'], 4:['C'], 5:['V','I'],
                6:['T'], 7:list(set(rc.restypes) - set(['G','A','S','C','V','I','T']))
                }
    sc_atom = {1:"CA", 2:"CB", 3:"OG", 4:"SG", 5:"CG1", 6:"OG1", 7:"CG"}
    zero_coord = torch.zeros_like(all_atom_positions[..., 1, :])
    # pdb.set_trace()
    pseudo_beta = zero_coord
    for i in range(1,8):
        cur_reses = aa_group[i]
        res_index = [rc.restype_order[res] for res in cur_reses]
        is_res = torch.zeros_like(aatype == 0)
        for res in res_index:
            is_res = (aatype == res) | is_res
        sc_atom_idx = rc.atom_order[sc_atom[i]]
        pseudo_beta += torch.where(
            is_res[..., None].expand(*((-1,) * len(is_res.shape)), 3),
            all_atom_positions[..., sc_atom_idx, :],
            zero_coord,
        )
...
def build_decoy_angle_feats(batch, all_atom_prev):
    decoy_feats = {}
    decoy_feats["decoy_aatype"] = batch["aatype"]
    decoy_feats["decoy_all_atom_positions"] = all_atom_prev
    decoy_feats["decoy_all_atom_mask"] = batch["atom37_atom_exists"]
    # pdb.set_trace()
    torsion_func = atom37_to_torsion_angles("decoy_")
    decoy_feats = torsion_func(decoy_feats)
    if decoy_feats is None:
        logging.warning(f"Some error occurs in atom37_to_torsion_angles, the all_atom_prev is {all_atom_prev}")
        return None
    decoy_aatype = batch["aatype"]
...
def build_decoy_pair_feats(
    batch, all_atom_prev, min_bin, max_bin, no_bins, eps=1e-20, inf=1e8
):
    # decoy_mask = batch["pseudo_beta_mask"]

    # Compute distogram (this seems to differ slightly from Alg. 5)
    tpb, decoy_mask = pseudo_beta_fn(batch["aatype"], all_atom_prev, batch["atom37_atom_exists"])
    ...
    to_concat = [dgram, decoy_mask_2d[..., None]]
    ...
    n, ca, c = [rc.atom_order[a] for a in ["N", "CA", "C"]]
    rigids = Rigid.make_transform_from_reference(
        n_xyz=all_atom_prev[..., n, :],
        ca_xyz=all_atom_prev[..., ca, :],
        c_xyz=all_atom_prev[..., c, :],
        eps=eps,
    )
    points = rigids.get_trans()[..., None, :, :]
    rigid_vec = rigids[..., None].invert_apply(points)
    ...
    d_aa_masks = batch["atom37_atom_exists"]
    decoy_mask = (
