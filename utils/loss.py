...
import pdb
######################################################################################
#Part 0: Loss Utils and Metric probably associated with loss(plddt or metric learning)
...
def sigmoid_cross_entropy(logits, labels):
    eps = logits.new_ones(size=logits.shape, requires_grad=False)*1e-10
...
def lddt_ca(
    ...
    # logging.warning(f"the shape of lddt loss is {lddt_score.shape}")
    lddt_score = torch.mean(lddt_score)
    return lddt_score
...
def supervised_chi_loss(
    ...
    decoy_aatype: torch.Tensor,
    decoy_seq_mask: torch.Tensor,
    label_chi_mask: torch.Tensor,
    label_chi_angles_sin_cos: torch.Tensor,
    chi_weight: float,
    ...
    **kwargs,
    ...
    true_chi = label_chi_angles_sin_cos[None]
    ...
    sq_chi_loss = masked_mean(
        label_chi_mask[..., None, :, :], sq_chi_error, dim=(-1, -2, -3)
    )
    ...
    angle_norm_loss = masked_mean(
        decoy_seq_mask[..., None, :, None], norm_error, dim=(-1, -2, -3)
    )
...
#Part 2: Frame Aligned Points Error
def compute_fape(
    pred_frames: Rigid,
    target_frames: Rigid,
...
def backbone_loss(
    label_backbone_rigid_tensor: torch.Tensor,
    label_backbone_rigid_mask: torch.Tensor,
    traj: torch.Tensor,
    ...
    **kwargs,
    ...
    # it might be fine.
    gt_aff = Rigid.from_tensor_4x4(label_backbone_rigid_tensor)

    fape_loss = compute_fape(
        pred_aff,
        gt_aff[None],
        label_backbone_rigid_mask[None],
        pred_aff.get_trans(),
...
def sidechain_loss(
    ...
    label_rigidgroups_gt_frames: torch.Tensor,
    label_rigidgroups_alt_gt_frames: torch.Tensor,
    label_rigidgroups_gt_exists: torch.Tensor,
    ...
    **kwargs,
    ...
    ) * label_rigidgroups_gt_frames + alt_naming_is_better[
        ..., None, None, None
    ] * label_rigidgroups_alt_gt_frames
    ...
    label_rigidgroups_gt_exists = label_rigidgroups_gt_exists.reshape(*batch_dims, -1)
...
######################################################################################
# Part3 Violation Loss: To penalize the structure violation and inject the domain knowledge into the model
...
def find_structural_violations(
    ...
    **kwargs,
    ...
    connection_violations = between_residue_bond_loss(
        ...
        pred_atom_mask=batch["decoy_atom14_atom_exists"],
        residue_index=batch["decoy_residue_index"],
        aatype=batch["decoy_aatype"],
    ...
    atom14_atom_radius = (
        batch["decoy_atom14_atom_exists"]
        * atomtype_radius[batch["decoy_residx_atom14_to_atom37"]]
    )

    # Compute the between residue clash loss.
    between_residue_clashes = between_residue_clash_loss(
        atom14_pred_positions=atom14_pred_positions,
        atom14_atom_exists=batch["decoy_atom14_atom_exists"],
...
def violation_loss(
    violations: Dict[str, torch.Tensor],
    decoy_atom14_atom_exists: torch.Tensor,
    eps=1e-6,
    **kwargs,
) -> torch.Tensor:
    num_atoms = torch.sum(decoy_atom14_atom_exists)
    l_clash = torch.sum(
...
def compute_renamed_ground_truth(
    ...
    # pdb.set_trace()
    atom14_gt_positions = batch["label_atom14_gt_positions"]
    gt_dists = torch.sqrt(
        eps
        + torch.sum(
            (
    ...
    atom14_alt_gt_positions = batch["label_atom14_alt_gt_positions"]
    ...
    atom14_gt_exists = batch["label_atom14_gt_exists"]
    atom14_atom_is_ambiguous = batch["label_atom14_atom_is_ambiguous"]
    mask = (
...
class RefineDiffLoss(nn.Module):
    """Aggregation of the various losses above, Supervise the Denoising Procedure in the following aspects:
        1.torsion angle, especially chi angles: for the sake of modeling the full atoms
        2.Frame Aligned Point Error: core loss to supervise the structure denosing procedure
        3.Structure Violation: Add constrains to the denoised structure
    """
    ...
    def forward(self, out, batch, _return_breakdown=False):
        ...
        for loss_name, loss_fn in loss_fns.items():
            ...
            loss = loss_fn()
            # logging.warning(f"the shape of the loss {loss_name} is {loss.shape}")
        ...
        seq_len = torch.mean(batch["decoy_seq_length"].float())
        crop_len = batch["decoy_aatype"].shape[-1]
        # pdb.set_trace()
